...
def training_handler():
    ...
    input_image_dimensions(x_train)
...
def save_image(img_name,img_dir,save_dir):
    if save_dir == 'model_name':
        pass
    

def train_images(parameter_list):
    pass
...
# load cifar 10 data
def input_image_dimensions(x_train):
...
def save_original_images(x_test,img_rows,img_cols,channels):
    imgs = x_test[:100]
...
def save_grayscale_images(x_test):
    pass

def create_autoencoder_models():
    pass

def convert_to_gray(x_train, x_test):
    x_train_gray = rgb2gray(x_train)
    x_test_gray = rgb2gray(x_test)

def display_grayscale_images():
    pass

def setup_plot(display_plot_axis,plot_title, imgs, plot_savedirectory ,interpolation = 'none', cmap ='gray' ):
    plt.figure()
    plt.axis(display_plot_axis)
    plt.title(plot_title)
    plt.imshow(imgs,interpolation,cmap)
    plt.savefig(plot_savedirectory)
